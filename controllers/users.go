...
func LoginUser(user models.User) (models.User, error, int) {
	existUser := models.User{}
	stmt, err := database.DataBase.Prepare("SELECT id, username, email, password FROM users WHERE username = ?")
	if err != nil {
		return models.User{}, fmt.Errorf("error preparing statement: %w", err), http.StatusInternalServerError
	}
	...
	if err == sql.ErrNoRows {
		...
		return models.User{}, fmt.Errorf("error scanning row: %w", err), http.StatusInternalServerError
	}
	...
	return existUser, nil, http.StatusOK
}
